...
def render_radar(
    ...
    if registrator is not None and registrator.H is not None:
        transformed_xy = registrator.transform(xy)
    else:
...
def run_ball_detection(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    for frame in frame_generator:
        result = ball_detection_model(frame, verbose=False)[0]
        detections = sv.Detections.from_ultralytics(result)
...
def run_team_classification(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    for frame in frame_generator:
        ...
        annotated_frame = ELLIPSE_LABEL_ANNOTATOR.annotate(annotated_frame, detections, custom_color_lookup=color_lookup)
        yield annotated_frame
